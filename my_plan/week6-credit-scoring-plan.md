...
| Architecture | New module `scoring`, sibling to `financial`/`loan`. One pure, stateless `@Component` (`ScoreCalculator`, mirrors `RatioCalculator`/`RiskIndicatorEvaluator`) does only the band-matching/summation math — it takes a lightweight `Band(minValue, maxValue, points)` record, not the `ScorecardRule` entity, keeping it as decoupled from persistence as the Week 4/5 calculators. `ScoreService` (the transactional boundary) resolves all inputs — reads `FinancialRatioRepository`/`RiskIndicatorRepository`/`FinancialStatementRepository` (financial) and `LoanApplicationRepository` (loan) directly, same cross-module-repository pattern already used by `FinancialStatementService`. |
| Scorecard/rule administration (confirmed with the user — in scope for Week 6, not deferred) | A new `ScorecardAdminController`/`ScorecardAdminService` (separate from `ScoreService`, which only computes scores — keeps each service focused) exposes full CRUD: create/update/delete a `Scorecard`, activate one, and add/update/delete its `ScorecardRule` bands. There is no existing precedent for admin CRUD in this codebase (`LoanProductController` is read-only, seeded via Liquibase only) — this is new, not a copy of an existing module. |
| "Only one active scorecard" enforcement | Copies the exact mechanism already used for `underwriting_attempt.active` (`009-create-underwriting-attempt.sql`): a partial unique index `CREATE UNIQUE INDEX ... ON scorecard (active) WHERE active` at the DB level, plus the service deactivating the previously-active row before activating the new one in the same transaction — not a Java-only check. |
| Overlapping rule bands | Validated in `ScorecardAdminService` when a rule is created/updated (pairwise interval-overlap check against sibling rules for the same `(scorecardId, factorCode)`, `null` treated as ±infinity) — throws `OverlappingScorecardRuleException`. Kept as a Java-level check rather than a Postgres exclusion constraint (`EXCLUDE USING gist`): this repo has no existing use of that feature, and range validation here isn't on the hot path, so a service-level check matches "prefer simple code over clever code." |
| Deleting a scorecard | Blocked (`ScorecardInUseException`, 409) when the scorecard is currently `active`, or when any `Score` row already references it — checked explicitly in the service rather than surfacing a raw FK constraint violation as a 500. Deleting/editing rules on an *inactive, unused* scorecard is unrestricted. Editing rules on a scorecard that already has scores is allowed and does not retroactively change past scores, since `RiskFactor` rows freeze the `value`/`points` computed at scoring time — only future scoring is affected. |
| Scorecard name uniqueness | `scorecard.name` is `UNIQUE`; violating it throws `DuplicateScorecardNameException` (409), mirroring `DuplicateCustomerException`/`DuplicateKycCaseException`. |
| Scorecard range validation | `minScore <= baseScore <= maxScore` is checked in the service on create/update; violating it throws `InvalidScorecardRangeException` (422), mirroring `InvalidFinancialPeriodException`'s role for `FinancialPeriodRequest`. |
...
- `021-seed-default-scorecard.sql`: one seeded `scorecard` (`SME_STANDARD_V1`, `active=true`, `base_score=500`, `min_score=300`, `max_score=900`) plus its `scorecard_rule` bands for all six `ScoreFactorCode` values, documented inline as SQL comments (formula/threshold source = this plan's design table). This is a starting default, not a hard-coded ceiling — the new admin API can add further scorecards or edit this one afterward.
- `017-create-scorecard.sql` also adds `CREATE UNIQUE INDEX uq_scorecard_active ON scorecard (active) WHERE active;` — same partial-unique-index pattern as `uq_underwriting_attempt_active_per_case`.
...
- `exception`: `NoActiveScorecardException`, `DuplicateScoreException`, `ScoreNotFoundException`, `ScorecardNotFoundException`, `DuplicateScorecardNameException`, `InvalidScorecardRangeException`, `ScorecardRuleNotFoundException`, `OverlappingScorecardRuleException`, `ScorecardInUseException`.
- `ScoreService` (`@Service @Transactional`): `score(reference, statementId, analysisRunId)` — loads the `FinancialAnalysisRun` and its `FinancialRatio`/`RiskIndicator` rows, walks `FinancialStatement.applicationId` to load the `LoanApplication` for `requestDetails`, loads the active `Scorecard` + its rules, builds the `Map<ScoreFactorCode, BigDecimal>` input map (skipping unresolvable factors), calls `ScoreCalculator`, persists `Score` + `RiskFactor` rows, returns `ScoreResponse`. Also `list(reference, statementId, analysisRunId)` / `get(reference, statementId, scoreId)`.
...
- `ScorecardAdminService` (`@Service @Transactional`, separate from `ScoreService`): `create`/`update`/`delete`/`activate` for `Scorecard`; `addRule`/`updateRule`/`deleteRule` for `ScorecardRule`, with the range/overlap/in-use validation described above.
- `ScorecardAdminController`: `@RequestMapping("/api/scorecards")` (global config, not loan-scoped — mirrors `/api/loan-products`, not `/api/loans/{reference}/...`). `POST` (create, 201 + `Location`), `GET` (list), `GET /{scorecardId}` (one, with its rules), `PUT /{scorecardId}` (update name/baseScore/minScore/maxScore), `DELETE /{scorecardId}`, `POST /{scorecardId}/activate`, `POST /{scorecardId}/rules` (201), `PUT /{scorecardId}/rules/{ruleId}`, `DELETE /{scorecardId}/rules/{ruleId}`.
...
- `ScoringIntegrationTest` (new test class in the `scoring` package, same `@SpringBootTest` MockMvc style as `LoanApplicationIntegrationTest`) — submit + analyze a statement, then `POST .../score` returns a persisted score with the expected grade; scoring the same run again under the same scorecard returns `DuplicateScoreException`/409; a `PersonalLoanRequest` with `monthlyIncome`/`existingEmi` produces an `EMI_TO_INCOME` factor, a non-personal product without those keys does not (and scoring still succeeds without it); scoring with no active scorecard returns `NoActiveScorecardException`/409.
- `ScorecardAdminIntegrationTest` (new) — create/update/delete a scorecard; activating one deactivates the previously-active one (only one `active=true` survives); creating a rule with an overlapping range is rejected; deleting an active scorecard, or one with existing scores, is rejected; duplicate scorecard name is rejected.
